...
public class MovieFullTextSearchServlet extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            // Create word boundary matching conditions
            ...
            // Build matching conditions for each keyword
            for (int i = 0; i < keywords.length; i++) {
                if (i > 0) {
                    sqlBuilder.append(" AND ");
                }
                // For each keyword, match the following patterns:
                // 1. Title starts with keyword
                // 2. A word in title starts with keyword (has space before)
