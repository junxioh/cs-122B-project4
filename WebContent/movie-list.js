...
async function loadMovies(){
    const r = await fetch(`/fabflix-movies/movie-list?${buildParams()}`);
    if(r.status===401) return location.href="/fabflix-login/login.html";
...
form.addEventListener("submit",e=>{
    ...
    if (searchTitleVal &&
        ...
        fetch(`/fabflix-movies/api/movie-search?query=${encodeURIComponent(searchTitleVal)}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                if (data.length > 0 && !data[0].hasOwnProperty('year')) {
                    ...
                    return fetch(`/fabflix-movies/api/movies-by-ids?ids=${encodeURIComponent(JSON.stringify(ids))}`)
                        .then(response => {
                            if (!response.ok) {
        ...
        fetch(`/fabflix-movies/api/movie-full-search?query=${encodeURIComponent(searchTitleVal)}`)
...
async function loadBrowseOptions(){
    const r = await fetch("/fabflix-movies/movie-list?browseOptionsOnly=true");
    if(r.status===401) return location.href="/fabflix-login/login.html";
    const d = await r.json();
